...
Most sessions are a typo, a failing test, or a red CI job. `agent-debug` owns that (launch it as a host subagent when logs would fill the parent; `KIT_SKILLS_ONLY=1` loads `agent-debug` in chat instead). Open the full feature lifecycle only when RCA needs a new capability.
