...
|---------|------------------------------|
| `wk align` | Consumer handshake, host pointers, kit MCP, commit-msg (`--write` seeds missing AGENTS.md and pointers; `--mcp` composes kit default; `--owned --scan` for a worktree farm; `--json` for findings) |
