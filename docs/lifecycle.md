...
8. **Close the loop:** After the timebox, run [product-signal intake](/SOPs/product-signal-intake) (Session A `wk mcp posthog`, restore default, human gate, then Session B `wk mcp default`). Confirm or kill via `agent-user-stories` and `agent-prune`. Do not add a separate product-insights role.
