...
| ID | Finding | Proposed action | Status |
|----|---------|-----------------|--------|
| R5 | Role skills over ~150-line kit-review budget (`agent-prune`, `agent-orchestrator`, `agent-debug`) | `wk verify` fails new over-budget role skills; named skills are allowlisted until thinned (MZW-32). `agent-copy` thinned via skill-local `references/` (2026-09-05). Move remaining procedure back to SOPs | **Open** (gate in; thinning still open) |
