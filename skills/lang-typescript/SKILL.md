...
- **Strict typing** - Enable `strict`. Never use `any`, `as any`, or `as unknown as`. Reach for `unknown` plus a type guard/Zod parse, explicit generics, or `satisfies T`. Tests use typed fakes, `Partial<T>`, and `vi.fn<typeof impl>` — not `as any`. Vitest `expect.any(Number)` is a matcher, not a type `any`. This kit enforces `typescript/no-explicit-any` via **oxlint** (`pnpm lint`, CI + pre-commit). Consumer apps should enable the same rule (oxlint or ESLint) as an error so it cannot regress.
