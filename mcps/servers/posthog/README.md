...
Use the `posthog` profile (`wk mcp posthog --install` or `wk mcp posthog --project`). Do not stack it onto `default`. Restore `wk mcp default --install` (or `--project`) when the analytics session ends.
