...
- **Scripted gate:** [`.github/workflows/agent-evals.yml`](../.github/workflows/agent-evals.yml) and `kit eval ci` / `kit check` use the keyword driver. Cases tagged `requires-live` are skipped so paraphrases do not fail CI. PR jobs may inject provider secrets but still default `KIT_EVAL_MODEL` to `scripted`. `kit check` also runs Cloudflare ops, safety, self-correction, and terminal-fallback suites.
