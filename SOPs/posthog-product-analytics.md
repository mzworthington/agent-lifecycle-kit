...
Isolation children in a repo: `wk mcp posthog --project`, then `wk mcp restore --project` only. User-scope: `wk mcp posthog --install`, then `wk mcp default --install`. One profile only. Do not mix scopes.
