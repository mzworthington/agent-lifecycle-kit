...
Install once per session: `kit mcp cloudflare-ops --install` (OAuth on first Cloudflare tool use). One profile only - do not stack `cloud` + `ops`. Restore `wk mcp restore --project` (or `wk mcp default --project`) when the session ends.
