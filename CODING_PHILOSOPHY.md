...
- **Explicit types:** Never use TypeScript `any`, `as any`, or `as unknown as`. Prefer `unknown` with narrowing, generics, or `satisfies`. Tests use typed fakes/`Partial<T>` — not `as any`. Vitest `expect.any(...)` matchers are not a type `any`. Enforce with `typescript/no-explicit-any` (oxlint in this kit: `pnpm lint`).
